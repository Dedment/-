...
function mod:EPAward(event_name, name, reason, amount, mass)
  ...
  mod:Announce(L["%+d EP (%s) to %s (%d)"], amount, reason, name, (EPGP:GetEPGP(name) or 0) + amount)
end

function mod:GPAward(event_name, name, reason, amount, mass)
  if mass then return end
  mod:Announce(L["%+d GP (%s) to %s (%d)"], amount, reason, name, (select(2, EPGP:GetEPGP(name)) or 0) + amount)
end
